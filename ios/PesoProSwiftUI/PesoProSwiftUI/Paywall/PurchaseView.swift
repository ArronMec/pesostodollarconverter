...
struct PurchaseView: View {
    ...
    let placeholderProductDetails: [PurchaseProductDetails] = [
        PurchaseProductDetails(price: "-", productId: "yearly_plan", duration: "year", durationPlanName: "Yearly Plan", hasTrial: false),
        PurchaseProductDetails(price: "-", productId: "weekly_plan", duration: "week", durationPlanName: "3-Day Trial", hasTrial: true)
    ]
    ...
    private func planHeadline(for productDetails: PurchaseProductDetails) -> String {
        return productDetails.durationPlanName
    }
    ...
    var calculatePercentageSaved: Int {
        if let calculateFullPrice = calculateFullPrice, let yearlyPriceString = purchaseModel.productDetails.first(where: {$0.duration == "year"})?.price {
            ...
            if let number = formatter.number(from: yearlyPriceString) {
                ...
                let saved = Int(round(100 - ((yearlyPriceDouble / calculateFullPrice) * 100)))
            ...
            VStack (spacing: 20) {
                ...
                VStack (spacing: 10) {
                    ...
                    VStack (alignment: .leading) {
                        PurchaseFeatureView(title: "Instantly convert MXN/USD", icon: "dollarsign.circle.fill", color: color)
                        PurchaseFeatureView(title: "No internet connection required", icon: "wifi", color: color)
                        PurchaseFeatureView(title: "24/7 live conversion rates", icon: "clock.arrow.2.circlepath", color: color)
                        ...
                        ForEach(productDetails) { productDetails in
                            ...
                            Button(action: {
                                ...
                                VStack {
                                    HStack {
                                        VStack(alignment: .leading) {
                                            Text(planHeadline(for: productDetails))
                                                .font(.headline.bold())
    ...
    var body: some View {
        ZStack {
            Circle()
                ...
                .shadow(color: Color.purple.opacity(0.5), radius: 20, x: 0, y: 20)
                .overlay(
                    Circle()
